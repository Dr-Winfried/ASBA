...
%TODO ab hier testweise keine einfache Quotierung - Effekt prüfen

\begingroup% Ab hier einfache Quotierung von Objekten, Zeichen und Zeichenketten
% Vorübergehende Neudefinition der Quotierung von
% Objekten                          [ob], Zeichen [char] und Zeichenketten [str]
% Relationen, Operatoren, Operanden [op], Symbolen [sym] und Formeln       [for]
\renewcommand*{\objqt }[1]          {#1}%                  Objekte
\renewcommand*{\charqt}[1]{\enquote*{#1}}%                 Zeichen
\renewcommand*{\strqt }[1]{\enquote {#1}}%                 Zeichenketten
...
Natürlich muss auch der Zusammenhang, in dem ein \glsIdx{MetaausdruckV} formuliert wird, bekannt sein, denn \textzB\ ist die Bedeutung von \enquote{Ich} nur dann bekannt, wenn man weiss von wem die Aussage ist.
Auf eine exakte Definition von \enquote{\glsIdx{MetaausdruckV}} wird verzichtet, weil das intuitive Verständnis hier ausreicht.
...
\end{align}
\footnote{%
	Insbesondere bei den Schlussregeln (\vrefseesec{sec:Schlussregeln}) ist es üblich, das Symbol \symqt{\srand} statt \symqt{\metaandsym} (bei niedrigerer Priorität (\vrefseetab{tab:Prio-Aussagenlogik})) zu verwenden.%
}
Offensichtlich sind das alles ebenfalls \glsIdxPl{MetaausdruckV}, jetzt aber teilweise formalisiert.
(c) lässt sich dann ausdrücken als \enquote{\strqt{Ich habe ein rotes Auto} $\metaandsym$ \strqt{das kann 200\,km/h schnell fahren.}}.
...
\opqt{A} und \opqt{B} können als Operanden von \opqt{\metaequiv}, \opqt{\metaandsym}, \opqt{\metaorsym} und \opqt{\srand} ohne Bedeutungsveränderung vertauscht werden.
...
\opqt{\metaimp}, \opqt{\metarep}, \opqt{\metaequiv}, \opqt{\metaandsym}, \opqt{\metaorsym} und \opqt{\srand} heißen \emph{\glsIdxPl{MetaoperatorV}}.
...
\begin{itemize}
	...
	%
	\item Wenn die Relationen \opqt{=} und \opqt{\equiv} beide verwendet werden, müssen ihre jeweiligen \emph{\glsIdxPl{intEigenschaftA}} bekannt sein.
	Dies wird immer vorausgesetzt und außerdem, dass jede \glsIdx{intEigenschaftA} für \opqt{\equiv} auch eine für \opqt{=} ist.
	Daraus folgt insbesondere, dass mit \forqt{(A \eq B)} auch \forqt{(A \equiv B)} gilt.
	%
	\item Bei Verwendung der Relationen \opqt{=}, \opqt{\ne}, \opqt{\equiv} und \opqt{\nequiv} müssen die jeweiligen Operanden \glsIdx{vergleichbar} sein. Dies wird immer vorausgesetzt.
	%
\end{itemize}
...
\footnote{%
	Die Bezeichnung \strqt{Operator} passt nicht mehr, da die Symbole \symqt{\eq}, \symqt{\neq}, \symqt{\equiv} und \symqt{\nequiv} \emph{Relationen} repräsentieren. \symqt{\metadefeq} und \symqt{\defeq} hingegen repräsentieren (zusammen mit den Operanden) Definitionen.
}
definiert:
\begin{description}
	%
	\item[\opqt{\glsSym{eq}}~~\emph{\Idx{Gleichheit}}]\label{def:Gleichheit}
	\forqt{A = B} heißt, dass \opqt{A} und \opqt{B} sich in den \glsIdxPl{intEigenschaftA} für \opqt{=} nicht unterscheiden.%
	\footnote{%
	...
	%
	\item[\opqt{\glsSym{ne}}~~\emph{\Idx{Ungleichheit}}]\label{def:Ungleichheit}
	\forqt{A \ne B} heißt, dass $A$ und $B$ sich in mindestens einer der \glsIdxPl{intEigenschaftA} für \opqt{=} unterscheiden. \strqt{$A$ ist \emph{nicht dasselbe} wie $B$} (aber vielleicht das gleiche) oder \strqt{$A$ ist \emph{nicht identisch} zu $B$}.
	%
	\item[\opqt{\glsSym{equiv}}~~\emph{\Idx{Äquivalenz}}]\label{def:Äquivalenz}
	\forqt{A \equiv B} heißt, dass $A$ und $B$ sich in den \glsIdxPl{intEigenschaftA} für \symqt{\equiv} nicht unterscheiden.
	-- \strqt{$A$ ist \emph{das gleiche} wie $B$} (aber vielleicht nicht dasselbe) oder \strqt{$A$ ist \emph{so wie} $B$}.
	%
	\item[\opqt{\glsSym{nequiv}}~~\emph{\Idx{Kontravalenz}}]\label{def:Kontravalenz}
	\forqt{A \nequiv B} heißt, dass $A$ und $B$ sich in mindestens einer der \glsIdxPl{intEigenschaftA} für \opqt{\nequiv} unterscheiden.
	-- \strqt{$A$ ist \emph{nicht das gleiche} wie $B$} oder \strqt{$A$ ist \emph{nicht so wie} $B$}.
	%
	\item[\opqt{\glsSym{metadefeq}}~\emph{\Idx{Metadefinition}}]\label{def:Metadefinition}
	...
	$A$ und $B$ können sich gegenseitig ersetzten.%
	\footnote{%
		Nach den Definitionen von \opqt{\metadefeq} und \opqt{\defeq} sind zwei Ausdrücke $P$ und $Q$ schon dann gleich, wenn nach der Ersetzung aller Vorkommen von $A$ durch $B$ sowohl in $P$ als auch in $Q$ die resultierenden Ausdrücke $\overline{P}$ und $\overline{Q}$ gleich sind.%
	}
	$B$ darf dabei von $A$ weder direkt noch indirekt abhängen, \textdh\ $A$ darf in $B$ und zugehörigen Definitionen noch nicht vorkommen.
	-- Man beachte, dass \opqt{\metadefeq} und \opqt{\defeq} verschiedene Sprachebenen sind.
...
%%%Es sei noch
%%%\[
%%%	\label{eq:asM} \glsSym{alMet} \quad \defeq \quad \{ \text{\symqt{\metaimp}, \symqt{\metarep}, \symqt{\metaequiv}, \symqt{\metaandsym}, \symqt{\metaorsym}, \symqt{\srand}, \symqt{\eq}, \symqt{\ne}, \symqt{\equiv}, \symqt{\nequiv}, \symqt{\metadefeq}, \symqt{\defeq}} \}
%%%\]
%%%die Menge der metasprachlichen Symbole.

Sollen zwei \glsIdxPl{MetaausdruckV} mittels der Relationen \opqt{=}, \opqt{\ne}, \opqt{\equiv} oder \opqt{\nequiv} miteinander verglichen werden, muss klar sein auf welche Art; \textzB\ als Zeichenfolgen -- mit oder ohne Wertung der Zwischenräume --, als \glsIdxPl{Wahrheitswert} oder auf sonstige Art.
Entsprechendes gilt für Definitionen mittels \opqt{\metadefeq} oder \opqt{\defeq}.

Wenn die Art des Vergleichs implizit oder explizit klar ist und sich zwei Ausdrücke auf diese Art vergleichen lassen, heißen sie \emph{\glsIdx{vergleichbar}}\vrefnotesec{sec:Formalelement}.

\endgroup%  Bis hier einfache Quotierung von Objekten, Zeichen und Zeichenketten
...
\end{align}
versuchen, aber die so definierte Relation \opqt{\relBsp} kann, muss aber nicht, mit \opqt{\relbsp} übereinstimmen, wie das Beispiel \vrefintab{tab:Gegenbeispiel} bei gegebenen Relationen \opqt{\releqbsp} und \opqt{\relbsp} zeigt.
...
Aus \eqref{eq:releqbsp} und \eqref{eq:relbsp} folgt allerdings
\begin{align}
...
Es sei noch angemerkt, dass wegen \eqref{eq:lrrelbsp} die Definition von \symqt{\metarep} \vrefinsub{sub:Metaausdruck} überflüssig ist und wegen der angegebenen Prioritäten \vrefintab{tab:Prio-Metasprache} auch alle Klammern in diesem \sectionname.
...
Für den Fall fehlender Klammern in (metasprachlichen) Ausdrücken und Formeln sind die Prioritäten \vrefintab{tab:Prio-Metasprache} angegeben.
...
\todo{Elimination von Voraussetzungen behandeln!}
\begin{itemize}
	\item[] eine Voraussetzung $\Voraussetzung \in \Voraussetzungen$ oder
	\item[] ein Ergebnis der Anwendung einer \emph{zulässigen \glsIdx{Schlussregel}}\footnote{%
...
Die damit bewiesene Aussage (\textzB\ ein mathematischer Satz) kann dann folgendermaßen formuliert werden:%
\footnote{%
	$\srand$ steht für "`und"', bindet aber wesentlich schwächer als $\metaand$. (\vrefseesub{sub:Metaausdruck})
}
\[
...
\]
Bevor die Schlussregeln behandelt werden, werden noch Elemente der \emph{\Idx{Aussagenlogik}} und der \emph{\Idx{Prädikatenlogik}} behandelt.
...
%
Durch Anwendung der Klammerregeln \vrefvonsubsub{subsub:Bausteine} lassen sich in der Regel noch viele Klammern der Formeln aus $\alForx$ einsparen.
...
Sie können dabei auch \emph{\glsIdx{atomareFormelA}n} bezeichnen, \textdh\ Formeln, die sich nicht weiter zerlegen lassen.\footnote{%
...
\end{itemize}
%
Es werden verschiedene Teilmengen von $\alJun$ -- \glsIdxPl{logischeSignaturV} -- eingeführt, die jeweils ausreichen alle anderen Elemente aus $\alJun$ zu definieren:
\begin{align}
...
Ausgehend von der \glsIdxBg{logischeSignaturV}{logischen Signatur} $\alJun_\xAnd = \{\lnot, \land\}$ und der \vrefdef{def:imp} von \symqt{\limp} werden die folgenden vier logischen Axiome definiert:
\begin{align}
	&

...
Die mathematischen Grundlagen werden einerseits gebraucht, um die erlaubten \glsIdxPl{Beweisschritt}\vrefnotesec{sub:Beweisschritte} zu definieren, andererseits dienen sie auch zum Testen von \ASBA.
...
\begin{description}
	...
	Einzelne Symbole werden \chrqt{so} quotiert.
	-- Die Quotierung ist kein Bestandteil des \glsIdx{Symbol}s!
	%
	\item [\glsIdxPl{Zeichenkette}] sind Folgen von \glsIdxPl{Symbol}, in denen auch Leerstellen erlaubt sind.
	...
	werden Zeichenketten stets \strqt{in dieser} Schriftart und \strqt{dieser} Quotierung dargestellt.
	-- Die Quotierung ist kein Bestandteil der \glsIdx{Zeichenkette}!
	%
	\item [\glsIdxPl{Zeichenfolge}] sind ähnlich wie \glsIdxPl{Zeichenkette}, außer das Leerzeichen und andere Zwischenraumzeichen nicht zählen.
	...
	%
	\item [\glsIdxPl{Formel}] sind in diesem Dokument immer nach vorgegebenen Regeln aufgebaute \glsIdxPl{Zeichenfolge}%
	\footnote{%
	...
	Man kann eine \glsIdx{Formel} $\alpha$ auch dadurch charakterisieren, dass sie ein Element einer vorgegebenen Menge $\Formelmenge$ von \glsIdxPl{Zeichenfolge} ist.%
	\footnote{%
		$\alpha$ wird dann auch \defn{Wort} der \defn{Sprache} $\Formelmenge$ genannt - besonders, wenn die Elemente von $\Formelmenge$ \glsIdxPl{Zeichenkette} statt \glsIdxPl{Zeichenfolge} sind.
		Wir bleiben der Klarheit willen bei \enquote{Formel}.
	}
	%
	\item[\glsIdxPl{Objekt}] sind \textzB\ \glsIdxPl{Symbol}, \glsIdxPl{Zeichenkette}, \glsIdxPl{Zeichenfolge} und \glsIdxPl{Formel}, oder auch \glsIdxPl{Aussage}, Mengen, Zahlen, \textusw\ -- ganz allgemein reale oder gedachte Dinge an sich.
	Eine \glsIdx{Formel}, die nicht quotiert ist, steht für den Wert dieser \glsIdx{Formel}, der dann wieder ein \glsIdx{Objekt} ist.
	Entsprechend steht ein \glsIdx{Symbol}, das nicht quotiert ist, für das dadurch bezeichnete \glsIdxPl{Objekt}.
...
Man beachte noch, dass in \glsIdxPl{Formel} Worte in normaler (lateinischer) Schrift jeweils genau ein Symbol repräsentieren sollen.
...
Wenn man über eine Sprache spricht, braucht man eine zweite Sprache, die \defn{\glsIdx{Metasprache}}, in der \glsIdxPl{Aussage} über die erstere getroffen werden können.%
\footnote{%
...
In diesem Fall bezeichnen wir sie als \defn{\Idx{zerlegbar}}, ansonsten als \defn{\Idx{unzerlegbar}}.
...
\footnote{%
	...
	Bei den den logischen Operatoren $\land$ und $\lor$ müssen Kommutativität und Assoziativität durch \glsIdxPl{Axiom} gefordert werden.
...
Wenn sie als jeweiliges Ergebnis der beiden \glsIdxPl{Formel}, \textdh\ als \glsIdx{Objekt}, verglichen werden, dann ist (a) richtig.
...
\begin{description}
	...
	\seqqt{$A = B$} heißt, dass $A$ und $B$ sich in den \glsIdxBg{interessierende-Eigenschaft}{interessierenden Eigenschaften} für $=$ nicht unterscheiden.%
	\footnote{%
...
$\eq$, $\ne$, $\equiv$ und $\nequiv$ bezeichnen wir als  \defn{\glsIdxPl{Gleichheitsrelation}}.
...
Es sei $\Formelmenge$ eine Menge von \glsIdxPl{Formel}.%
	\footnote{%
	Die Menge der \glsIdxPl{Formel} wird auch als \defn{Sprache} (language) bezeichnet und die \glsIdxPl{Formel} sind dann die Worte dieser Sprache.
	Wir bleiben der Klarheit willen bei \glsIdx{Formel}.
	Dabei gibt es verschiedene Arten von \glsIdxPl{Formel}, \textzB\ aussagenlogische, prädikatenlogische, solche, die ein Taschenrechner auswerten kann, oder auch nach bestimmten Regeln gebildete Zeichenketten.
}
Dann definieren wir:
\begin{itemize}
	\item[] Eine \defn{\glsIdx{Schlussregel}} $\frac{\Voraussetzungsmenge}{\Folgerungsmenge}$ \emph{für} $\Formelmenge$ besteht aus einer Menge $\Voraussetzungsmenge\subseteq\Formelmenge$, den \defn{\glsIdxPl{Voraussetzung}}, und einer Menge $\Folgerungsmenge\subseteq\Formelmenge$, den \defn{\glsIdxPl{Folgerung}}.
\end{itemize}
...
Eine \glsIdxBg{allgemeingueltige-Schlussregel}{Schlussregel} \textbzw\ ein \glsIdx{formaler-Satz} heißt \defn{allgemeingültig}, wenn die zugehörige \glsIdx{Aussage} immer zutrifft.
...
\begin{itemize}
	...
	\item Eine \glsIdx{Schlussregel} $\frac{\emptyset}{\Folgerungsmenge}$ (oder kürzer $\Folgerungsmenge$), die keine Voraussetzungen hat, nennen wir ein \defn{\glsIdx{Axiomensystem}}.
	Ist $\Folgerungsmenge = \{\alpha\}$, so nennen wir die \glsIdx{Schlussregel} (oder kürzer $\alpha$) auch \defn{\glsIdx{Axiom}}.
\end{itemize}
...
wobei $K$ ein Element von $\gsNo$ ist, die \defn{\glsIdxPl{Beweisschritt}} $\Beweisschritt_k$ \glsIdxPl{Schlussregel} und die Transformationen später definiert werden.
Wir definieren noch:
\begin{align}
...
\end{align}
und nennen $\Beweisschrittmenge$ die \defn{\glsIdx{Beweisschrittmenge}} der \glsIdx{Beweisschrittfolge} $\Beweisschrittfolge$.
...
\begin{table}
	...
	\begin{threeparttable}
		...
		\begin{tablenotes}
			...
			%
			\item[2] 0-stellige Operatoren sind Konstante, hier \emph{\glsIdxPl{Wahrheitswert}}.
			%
...
-- auch \defn{Junktoren} genannt --, Namen und Sprechweisen sind auch Alternativen angegeben.
...
Sie können dabei auch als \defn{\glsIdx{atomare-Formel}} bezeichnet werden, \textdh\ \glsIdxPl{Formel}, die sich nicht weiter zerlegen lassen.%
\footnote{%

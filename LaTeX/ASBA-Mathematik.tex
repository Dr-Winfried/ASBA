...
\begin{align}
	...
	\\
	& A \Sym{\und}         B & \text{steht für }
	& \text{\strqt{$A$ \emph{und}  $B$}.}
	\formulatoleft
\end{align}
%
Insbesondere bei den Schlussregeln\vrefnotesec{sec:Schlussregeln} ist es üblich, \symqt{$\und$} statt \symqt{$\metaand$} (bei schwächerer Priorität\vrefnotetab{tab:Prio-Aussagenlogik}) zu verwenden.
%
...
Um Verwechslungen mit den logischen Symbolen zu vermeiden, werden für \strqt{und} und \strqt{oder} die Symbole \symqt{$\metaand$} und \symqt{$\metaor$} verwendet.
$A$ und $B$ können als Operanden von \symqt{$\metaequiv$}, \symqt{$\metaand$}, \symqt{$\metaor$} und \symqt{$\und$} ohne Bedeutungsveränderung vertauscht werden.
Wird in einer Formel nur einer der Operatoren \symqt{$\metaand$}, \symqt{$\metaor$} oder \symqt{$\und$} verwendet, können die Operanden beliebig permutiert werden, so dass dann auch eine Klammerung überflüssig ist.
...
\symqt{$\metaimp$}, \symqt{$\metarep$}, \symqt{$\metaequiv$}, \symqt{$\metaand$}, \symqt{$\metaor$} und \symqt{$\und$} heißen \emph{\glsIdxPl{MetaoperatorV}}.
...
\subsection{Mit Gleichheit verwandte Operatoren}% ------------------------------
...
\subsubsection{Definition von mit Gleichheit verwandten Operatoren}% - - - - - -
\label{subsub:DefinitionGleichheit}

Unter den Voraussetzungen \vrefvonsubsub{subsub:Voraussetzungen} werden die folgenden (metasprachlichen) Operatoren definiert:
\begin{description}
	%
...
Es sei noch
\[
	\label{eq:asM} \glsSym{asM} \quad \defeq \quad \{\; \metaimp \,,\; \metarep \,,\; \metaequiv \,,\; \metaand \,,\; \metaor \,,\; \und \,,\; \eq \,,\; \ne \,,\; \equiv \,,\; \notequiv \,,\; \metadefeq \,,\; \defeq \; \}
\]
...
Es sei noch angemerkt, dass wegen \eqref{eq:corel} die Definition von \symqt{$\metarep$} \vrefinsub{sub:Metaausdruck} überflüssig ist und wegen der Klammerregeln (\vrefseesub{sub:Klammerregeln}) auch alle Klammern in diesem \sectionname.
...
\footnote{%
	\symqt{$\und$} steht für \strqt{und}, bindet aber wesentlich schwächer als \symqt{\glsIdx{metaand}}. (\vrefseesub{sub:Metaausdruck})
}
\[
...
%%%\symqt{$\und$} für \strqt{und} \textbzgl\ \symqt{\glsIdx{metaand}}\vrefnotesub{sub:Metaausdruck} steht und schwächer bindet als \symqt{$\derive$} und \symqt{$\subst$}.%
...
%%%	\item Wenn $A_i$ nicht leer ist, ist bis jetzt die Schlussregel $\dfrac{A_{d_1} \und A_{d_2} \und ...}{A_i}$ (alle $d_j \in D_i$) schon bewiesen.
%%%\end{itemize}
...
%%%		\\\hline
%%%		11 & \tagnotb & & $\dfrac{X,\alpha \derive \beta \und X,\lnot\alpha \derive \beta}{X \derive \beta}$ & 2, 10 & 3 & 1, 2
%%%		\\\hline\hline
%%%		12 & \centerParbox{1.4cm}{\tagAR, \tagMR, \tagnota, \tagnotb} & & $\dfrac{A_1 \und A_2}{A_3}$ & & $\dfrac{X \derive \alpha \und X, \alpha \derive \beta}{X \derive \beta}$ &
%%%		\\\hline
%%%	\end{tabular}
...
%%%		\\\hline
%%%		10' & -1 & $X \subst \emptyset$ & $\dfrac{\alpha \und \beta}{\alpha \land \beta}$ & & &
%%%		\\\hline
%%%		11' & -1 &\SnCell{
%%%			$\alpha \swap \beta$\\
%%%			$\alpha \subst \lnot\beta$
%%%		}  & $\dfrac{\beta \und \lnot\beta}{\beta \land \lnot\beta}$ & 8, 3 & $\beta \land \lnot\beta$ &
%%%		\\\hline
%%%		9 & \tagnota & & $\dfrac{X \derive \alpha, \lnot\alpha}{X \derive \beta}$ & & &
%%%		\\\hline
%%%		10 & -1 & $X \subst \emptyset$ & $\dfrac{\alpha \und \lnot\alpha}{\beta}$ & & &
%%%		\\\hline
%%%		11 & -1 & \SnCell{
%%%			$\alpha \swap \beta$\\
%%%			$\alpha \subst \lnot\alpha$
%%%		} & $\dfrac{\beta \und \lnot\beta}{\lnot\alpha}$ & 8, 3 & $\lnot\alpha$ & 2, 3, 4
%%%		\\\hline
...
\footnote{%
	Im Gegensatz zu \vrefsubsub{subsub:Bausteine} können A und B hier beliebige Aussagen -- auch Formeln -- sein.%
},
...
\begin{table}
	...
	\begin{threeparttable}
		...
		\begin{tablenotes}
			...
			Der Einfachheit halber werden auch die beiden Konstanten $\ltrue$ und $\lfalse$ als Junktoren \textbzgl\ Operatoren bezeichnet.

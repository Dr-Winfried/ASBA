...
Ein Beweis in \ASBA\ besteht aus einer Menge von \glsIdx{formalesElementV} (die \emph{Voraussetzungen}) und einer Folge von \glsIdxPl{zulaessigeTransformationA}, mit der neue (Mengen von) \glsIdxBg{formalesElementV}{formalen Elementen} generiert werden, bis eine gewünschte Menge von \glsIdxBg{formalesElementV}{formalen Elementen} (die \emph{Folgerungen}) abgeleitet sind.

In diesem \sectionname\ geht es um die \glsIdxPl{zulaessigeTransformationA}, \textdh die \glsIdxBg{gueltigeSchlussregel}{gültigen Schlussregeln}. Dazu gehören zunächst die \glsIdxPl{Basisregel}.
...
Eine \emph{\glsIdx{zulaessigeTransformationA}} ist die Anwendung einer \emph{\glsIdx{Basisregel}} (siehe unten) oder einer davon abgeleiteten sonstigen \emph{\glsIdx{Schlussregel}}, \textzB aus \subsectionname~\vref{sub:Schlussregeln}.

Seien $\alpha$ und $\beta$ wieder \glsIdxPl{formalesElementV} und $X$ und $Y$ Mengen von \glsIdxBg{formalesElementV}{formalen Elementen}.
...
Bei den \glsIdxPl{Schlussregel} und der Substitution\footnote{\seename~\vref{sub:Indentitaetsregeln}} soll das Komma stärker binden als \symqt{$\abltb$} und \symqt{$\subst$}.
\symqt{$\subst$} steht für \strqt{und} \textbzw \symqt{\glsIdx{metaand}}\footnote{\seename\ \subsectionname~\vref{sub:Metaausdruck}} und bindet schwächer als \symqt{$\abltb$} und \symqt{$\subst$}.%
...
Eine Schlussregel steht für die Aussage, dass mit den \glsIdxBg{formalesElementV}{formalen Elementen}\footnote{hier: Aussagen in einer formalen Form.} über dem Strich auch die unter dem Strich gelten.
...
Dazu wird dem Gleichheits- oder Identitätszeichen \symqt{$\eq$} mittels Einführungs- und Beseitigungsregel eine Bedeutung verliehen.\footnote{\seename~\cite{bib:NatuerlichesSchliessen}}
...
\parbox[t]{10.5cm}{%
Das \glsIdxBg{formalesElementV}{formale Element}, dass man erhält, wenn in $\gamma$ alle oder nur einige Vorkommen von $\alpha$ durch $\beta$ ersetzt.
}
\label{def:Substitution}
\]
Gegebenenfalls muss noch die Auswahl der Ersetzungen angegeben werden.
...
\begin{align}
	...
	& & (\text{\glsIdx{Abtrennungsregel} (Modus ponens)})
...
%TODO Nachweis, dass X, Y, \alpha und \beta durch X', Y', \alpha' und \beta' ersetzt werden könnnen.
...
Dazu werden für die Zeile $n$ definiert:
\begin{align}
	R_n & \defeq
	\left\{
		\begin{array}{l}
			A_n \quad \text{, wenn in der Spalte \strqt{Regel} \strqt{...Annahme} steht.} \\
			A_i \quad \text{\textbzw } \overline{R}_i \quad \text{, wenn in der Spalte \strqt{Regel} die Zahl $i$ steht.} \\
			\text{sonst: die Schlussregel, auf die in der Spalte \strqt{Regel} verwiesen wird}
		\end{array}
	\right. \\
	S_n & \defeq \text{die Menge der Substitutionen in der Spalte \strqt{Substitutionen}.} \\
	Z_n & \defeq \text{die Menge der Zeilennummern in der Spalte \strqt{angewendet auf}.} \\
	\overline{R}_n&\defeq \text{das Ergebnis der Substitution auf die Regel \textbzw Aussage $R_n$.} \\
	A_n & \defeq
	\left\{
		\begin{array}{l}
			\overline{R}_n \quad \text{, wenn in der Spalte \strqt{Aussage} nichts steht.} \\
			\text{sonst: das \glsIdxBg{formalesElementV}{formale Element} in der Spalte \strqt{Aussage}.}
		\end{array}
	\right. \\
	D_n & \defeq \text{die Menge der Abhängigkeiten in der Spalte \strqt{Abhängigkeiten}.}
\end{align}
%%%Ein Element $i \in Z_n$ verweist auf $A_i$.
%%%Geht einer Substitution ein \strqt{$i:$} voraus, bezieht sie sich nur auf $A_i$ \textbzw $R_n$ für $i = n$, ansonsten auf alle $A_i$ mit $i \in Z_n$ (dann aber nicht auf $R_n$).

\begin{table}[!htb]
	\setlength\tabcolsep{2pt}
	\setlength\extrarowheight{9pt}
%%%	\begin{tabularx}{\linewidth-10.95pt}{|c||l|l|l|l|l|l|}
	\begin{tabular}{|c||c|c|c|c|c|c|}
		\hline
		\parbox{1.0cm}{\textbf{Zeile         }\newline                    ($n$)} &
		\parbox{1.1cm}{\textbf{Regel         }\newline                  ($R_n$)} &
		\parbox{2.8cm}{\textbf{Substitutionen}\newline                  ($S_n$)} &
		\parbox{2.7cm}{\textbf{erzeugte Regel}\newline       ($\overline{R}_n$)} &
		\parbox{2.3cm}{\textbf{angewendet    }\newline \textbf{auf ...} ($Z_n$)} &
		\parbox{1.6cm}{\textbf{Aussage       }\newline                  ($A_n$)} &
		\parbox{2.0cm}{\textbf{Abhängig-     }\newline \textbf{keiten}  ($D_n$)} \\
		\hline \hline
		1 & fixe Annahme & & & & $ X \abltb \alpha $ & 1 \\
		\hline
		2 & fixe Annahme & & & & $ X,\alpha \abltb \beta $ & 2 \\
		\hline
		3 & Annahme & & & & $ X \abltb \beta $ & 3 \\
		\hline
		4 & \glsIdx{MR} & $ Y \subst X,\lnot\alpha $ & $ \dfrac{X\abltb\alpha}{X,\lnot\alpha\abltb\alpha} $ & 1 & $ X,\lnot\alpha \abltb \alpha $ & 1 \\
		\hline
		%TODO Beispiel vollenden
		5 & \glsIdx{AR} & $ \alpha \subst \gamma $ & $ \dfrac{}{\gamma \abltb \gamma} $ & & $ \gamma \abltb \gamma $ & \\
		\hline
		6 & \glsIdx{MR} & \parbox{2.3cm}{
			$ \alpha \subst \gamma $ \newline
			$ X \subst \gamma $      \newline
			$ Y \subst X,\gamma $
		} & $ \dfrac{\gamma\abltb\gamma}{X,\gamma\abltb\gamma} $ & 5 & $ X,\gamma\abltb\gamma $ & \\
		\hline
		7 & 6 & $ \gamma \subst \lnot\alpha $ & $ X,\lnot\alpha \abltb \lnot\alpha $ & & & \\
		\hline
		8 & \glsIdx{nota} & $ X \subst Z $ & $ \dfrac{Z\abltb\alpha,\lnot\alpha}{Z\abltb\beta} $ & & & \\
		\hline
		9 & 8 & $ Z \subst X,\lnot\alpha $ & $ \dfrac{X,\lnot\alpha\abltb\alpha,\lnot\alpha}{X,\lnot\alpha\abltb\beta} $ & 4, 7 & $ X,\lnot\alpha \abltb \beta $ & 1 \\
		\hline
		10 & \glsIdx{notb} & & $ \dfrac{X,\alpha\abltb\beta \und X,\lnot\alpha\abltb\beta}{X\abltb\beta} $ & 2, 9 & $ X \abltb \beta $ & 1, 2 \\
		\hline
	\end{tabular}
%%%	\end{tabularx}
	\caption{Ableitung der \glsIdx{Schnittregel} aus den \glsIdxPl{Basisregel}}
	\label{tab:Beispielableitung}
\end{table}

Eine Zeile $n$ in der \tablename~\vref{tab:Beispielableitung} hat folgende Bedeutung:
\begin{itemize}
	\item Nehme die Regel/Aussage $R_n$ und wende darauf die Substitutionen aus $S_n$ an. Das Ergebnis ist $\overline{R}_n$.
	\item Wende die Regel/Aussage $\overline{R}_n$ auf die Aussagen $A_i$ mit $i \in Z_n$ an.
	\item Die Aussage (unter dem Strich von) $\overline{R}_n$ bildet die (erzeugte) Aussage $A_n$.
	\item Die Menge $D_n$ der Abhängigkeiten ist gleich der Vereinigung der Mengen $D_i$ mit $i \in Z_n$.%
	...
	\begin{center}
		\begin{threeparttable}
			...
			\begin{tablenotes}
				...
				\item[2] \seename\ \subsectionname~\vref{sub:Metaausdruck} und für \symqt{$\subst$} \subsectionname~\vref{sub:Basisregeln}

...
%%%		\textZB\ könnte man eine 2$\times$2--Matrix auch schreiben als Folge von Zeilen: \seqqt{$[(a,b),(c,d)]$}, oder noch einfacher: \seqqt{$[a,b;c,d]$}.
	...
	Man kann eine \glsIdx{Formel} $\alpha$ auch dadurch charakterisieren, dass sie ein Element einer vorgegebenen Menge $\formulaset$ von \glsIdxPl{Zeichenfolge} ist.%
	\footnote{%
		$\alpha$ wird dann auch \defn{Wort} der \defn{Sprache} $\formulaset$ genannt - besonders, wenn die Elemente von $\formulaset$ \glsIdxPl{Zeichenkette} statt \glsIdxPl{Zeichenfolge} sind.
		Wir bleiben der Klarheit willen bei \enquote{Formel}.
	}
	%
	\item[\glsIdxPl{Objekt}] sind \textzB\ \glsIdxPl{Symbol}, \glsIdxPl{Zeichenkette}, \glsIdxPl{Zeichenfolge} und \glsIdxPl{Formel}, oder auch \Aussagen, Mengen, Zahlen, \textusw\ -- ganz allgemein reale oder gedachte Dinge an sich.
...
Wenn man über eine Sprache spricht, braucht man eine zweite Sprache, die \defn{\glsIdx{Metasprache}}, in der \Aussagen\ über die erstere getroffen werden können.%
\footnote{%
...
\footnote{%
	...
	So wird \textzB\ aus der \glsIdx{Formel} \seqqt{$A<B$} die \Aussage\ \enquote{$A$ ist kleiner als $B$}.
}
...
Es sei $\formulaset$ eine Menge von \glsIdxPl{Formel}.%
	\footnote{%
...
Dann definieren wir:
\begin{itemize}
	\item[] Eine \defn{\glsIdx{Schlussregel}} $\frac{\prerequisiteset}{\conclusionset}$ \emph{für} $\formulaset$ besteht aus einer Menge $\prerequisiteset\subseteq\formulaset$, den \defn{\glsIdxPl{Voraussetzung}}, und einer Menge $\conclusionset\subseteq\formulaset$, den \defn{\glsIdxPl{Folgerung}}.
\end{itemize}
Eine Schlussregel $\frac{\prerequisiteset}{\conclusionset}$ entspricht der \Aussage:
%
%TODO Verweis auf zulässige Transformationen
\begin{itemize}
	\item[] Mit den \glsIdxPl{Voraussetzung} aus $\prerequisiteset$ lassen sich alle \glsIdxPl{Folgerung} aus $\conclusionset$ ableiten%
...
Eine \glsIdxBg{allgemeingueltige-Schlussregel}{Schlussregel} \textbzw\ ein \glsIdx{formaler-Satz} heißt \defn{allgemeingültig}, wenn die zugehörige \Aussage\ immer zutrifft.
...
wobei $K$ ein Element von $\gsNo$ ist, die \defn{\Beweisschritte} $\Beweisschritt_k$ \glsIdxPl{Schlussregel} und die Transformationen später definiert werden.
Wir definieren noch:
\begin{align}
...
\end{align}
und nennen $\proofstepset$ die \defn{\glsIdx{Beweisschrittmenge}} der \glsIdx{Beweisschrittfolge} $\proofstepsequenz$.
Dann ist $\proofstepset_0=\emptyset$ und $\proofstepset_i\subseteq\proofstepset_j\subseteq\proofstepset$ für $0\le i\le j\le K$.

========================================================================
%TODO===================================================================

Jeder \Beweisschritt\ $ \Beweisschritt_k \text{ für } 1 \le k \le K $ muss entweder eine \glsIdx{Voraussetzung} aus $\prerequisiteset$ oder durch Anwendung einer \allgemeingueltigenSchlussregel\ auf eine Teilmenge von $\proofstepset_{k-1}$ eine wahre Formel oder eine weitere \emph{allgemeingueltige Schlussregel} sein.
...
\footnote{%
	$A$ und $B$ können hier beliebige \Aussagen\ sein -- auch \glsIdxPl{Formel} --, die jeweils genau einen \glsIdx{Wahrheitswert} repräsentieren.
}
...
Wenn für eine bestimmte Kombination von \glsIdxPl{Wahrheitswert}n mehr als eine Zeile angegeben ist, so können die zugehörigen Operatoren zwar formal verschieden sein, liefern in der zweiwertigen \Aussagenlogik\ jedoch dieselben Ergebnisse.
...
Sie können dabei auch als \defn{\atomareFormel} bezeichnet werden, \textdh\ \glsIdxPl{Formel}, die sich nicht weiter zerlegen lassen.%
\footnote{%

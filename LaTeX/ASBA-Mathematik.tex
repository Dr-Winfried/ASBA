...
\begin{description}
	...
	Für eine \defn{binären}, \textdh\ 2-stellige \Operation\ $\opbsp$ schreibt man \textiAlg\ \enquote{$x \opbsp y$} statt \enquote{$\opbsp(x,y)$}.
	...
	Für eine \defn{binäre}, \textdh\ 2-stellige \Relation\ $\relbsp$ schreibt man \textiAlg\ \enquote{$x \relbsp y$} statt \enquote{$(x,y) \in\; \relbsp$}.
...
Besondere Vereinbarungen für \chrqt{$\opubsp$} treffen wir nicht.
...
In diesem Fall bezeichnen wir sie als \defn{\zerlegbar}, ansonsten als \defn{\unzerlegbar} oder auch \defn{\Idx{atomar}}.
...
\begin{description}
	%
	\item[$\glsSym{eq}$] \defn{\glsIdx{Gleichheit}} \label{def:Gleichheit}
	\seqqt{$A = B$} heißt, dass $A$ und $B$ in den \interessierendenEigenschaften\ für $\eq$ übereinstimmen.%
	\footnote{%
	...
	%
	\item[$\glsSym{equiv}$] \defn{\glsIdx{Aequivalenz}} \label{def:Aequivalenz}
...
\Gleichheit\ und \Aequivalenz\ sind \defn{\emph}{Äquivalenzrelationen}, \textdh\ sie sind \emph{reflexiv} ($a \sim a$), \emph{transitiv} ($((a \sim b) \metaand (b \sim c)) \metaimp (a \sim c)$) und \emph{symmetrisch} ($(a \sim b) \metaimp (b \sim a)$)
...
\glsIdx{alForpx} sei die Menge der auf folgende Weise definierten aussagenlogischen \Formeln\ in \emph{Polnischer Notation}%

...
\begin{align}
	...
	& \quad\;\; \text{auch in Form von einem oder mehreren $Aj$ (mit $j < i$).}\\
	& \quad\;\; \text{In der Ergebniszeile kann hier auch die bewiesene Aussage als Schlussregel stehen.}
	\\
	D_i & \defeq \text{Die Verweise auf die Regeln, von denen $\overline{R}_i$ und}\\
	& \quad \;\; \text{die Indizes der $A_j$, von denen $A_i$, abhängig sind.}
\end{align}
...
\begin{table}[!htb]
	...
	\begin{tabular}{|c||c|c|c|c|c|c|}
		...
		\\\hline
		4 & \tagMR & & $\dfrac{X \derive \alpha}{X, Y \derive \alpha}$ & & & \tagMR
		\\\hline
		5 & 4 & $Y \subst \lnot\alpha$ & $\dfrac{X \derive \alpha}{X, \lnot\alpha \derive \alpha}$ & 1 & $X, \lnot\alpha \derive \alpha$ & \centerParbox{1, \tagMR}
		\\\hline
		6 & \tagAR & & $ \dfrac{}{\alpha \derive \alpha} $ & & & \tagAR
		\\\hline
		7 & 6 & $\alpha \subst \lnot\alpha$ & $\dfrac{}{\lnot\alpha \derive \lnot\alpha}$ & & $\lnot\alpha \derive \lnot\alpha$ & \tagAR
		\\\hline
		8 & 4 & $\alpha \subst \lnot\alpha$ & $\dfrac{X \derive \lnot\alpha}{X, Y \derive \lnot\alpha}$ & & & \tagMR
		\\\hline
		9 & 8 & $X \subst \lnot\alpha$ & $\dfrac{\lnot\alpha \derive \lnot\alpha}{Y, \lnot\alpha \derive \lnot\alpha}$ & & & \tagMR
		\\\hline
		10 & 9 & $Y \subst X$ & $\dfrac{\lnot\alpha \derive \lnot\alpha}{X,\lnot\alpha \derive \lnot\alpha}$ & 7 & $X,\lnot\alpha \derive \lnot\alpha$ & \centerParbox{\tagAR, \tagMR}
		\\\hline
		11 & \tagnota & & $\dfrac{X \derive \alpha, \lnot\alpha}{X \derive \beta}$ & & & \tagnota
		\\\hline
		12 & 11 & $X \subst X, \lnot\alpha$ & $\dfrac{X,\lnot\alpha \derive \alpha, \lnot\alpha}{X,\lnot\alpha \derive \beta}$ & 5, 10 & $X,\lnot\alpha \derive \beta$ & \centerParbox{1, \tagAR, \tagMR, \tagnota}
		\\\hline
		13 & \tagnotb & & $\dfrac{X,\alpha \derive \beta \und X,\lnot\alpha \derive \beta}{X \derive \beta}$ & 2, 12 & $X \derive \beta$ & \centerParbox{1, 2, \tagAR, \tagMR, \tagnota, \tagnotb}
		\\\hline\hline
		14 & & & $\dfrac{A_1 \und A_2}{A_3}$ & & $\dfrac{X \derive \alpha \und X, \alpha \derive \beta}{X \derive \beta}$ & \centerParbox{\tagAR, \tagMR, \tagnota, \tagnotb}
		\\\hline
	\end{tabular}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item $R_i =$ \strqt{Voraussetzung} \textbzw \strqt{Folgerung}.
		\item $A_i =$ Die aktuelle Voraussetzung \textbzw Folgerung.
	...
	\begin{enumerate}
		%
		\item \label{item:setDvonR} $R_i =
		\left\{
			\begin{array}{l}
				\text{- \strqt{Annahme} = Die Aussage $A_i$ wird vorübergehend angenommen.}\\
				\qquad \text{In dieser Zeile wird nur noch $A_i$ und $D_i$ = $i$ ausgefüllt.}\\
				\text{- $j$ = Die Schlussregel $\overline{R}_j$ für ein $j < i$ soll angewendet werden.}\\
				\qquad \text{Die Abhängigkeiten $D_j$ werden den Abhängigkeiten $D_i$ hinzugefügt.}\\
				\qquad \text{(\GlsIdxPl{Schlussregel} können nur von Schlussregeln abhängig sein.)}\\
				\text{- Verweis (ohne Klammern) auf eine \glsIdx{allgemeingueltigeSchlussregelV}.}\\
				\qquad \text{Der Verweis auf diese Schlussregel wird nach $D_i$ übernommen.}
			\end{array}
		\right.$
		%
		...
		Man beachte, dass die Schlussregel $\overline{R}_i$, falls angegeben, stets allgemeingültig ist, da sie ausschließlich aus \glsIdxBg{allgemeingueltigeSchlussregelV}{allgemeingültigen Schlussregeln} mittels Substitutionen abgeleitet ist.
		Daher gibt es auch keine Beschränkung weiterer Substitutionen durch irgendwelche Abhängigkeiten.
		%
		\item \label{item:setDvonZ} $Z_n =$ Die Indizes aller $A_j$ (mit $j < i$), die eine Voraussetzung der Schlussregel $\overline{R}_i$ sind.\newline
		Für jedes angegebene $j$ werden die Abhängigkeiten $D_j$ den Abhängigkeiten $D_i$ hinzugefügt.
		%
		\item $A_i =$ Folgerung(en) der Schlussregel $\overline{R}_i$.
		%
		\item $D_i =$ Die Verweise wurden schon in (\ref{item:setDvonR}) und (\ref{item:setDvonZ}) eingetragen.%
		\footnote{Wenn $D_n$ leer ist, dann ist $A_n$ allgemeingültig.}
		%
	\end{enumerate}
	Der Beweis muss so lange fortgeführt werden, bis alle Folgerungen abgeleitet worden sind.
	%
...
\begin{table}[!htb]
	...
	\begin{tabular}{|c||c|c|c|c|c|c|}
		...
		\titleCell{1.95cm}{Abhängig"=keiten}          {$D_n$}
		\\\hline \hline
		1 & \centerParbox[1.00cm]{Folge"=rung} & & & & $\derive (\alpha\limp\beta)\limp(\lnot\beta\limp\lnot\alpha)$ & 1
		\\\hline
		2 & \centerParbox[1.20cm]{An"=nahme} & & & & $\derive \alpha\limp\beta$ & 2
		\\\hline
		3 & \centerParbox[1.20cm]{An"=nahme} & & & & $\derive  \lnot\beta$ & 3
		\\\hline
		4 & \centerParbox[1.20cm]{An"=nahme} & & & & $\derive  \alpha$ & 4
		\\\hline
		5 & \tagimpB & & $\dfrac{X \derive \alpha\limp\beta}{X,\alpha \derive \beta}$ & & & \tagimpB
		\\\hline
		6 & -1 & $X \subst \emptyset$ & $\dfrac{\derive \alpha\limp\beta}{\alpha \derive \beta}$ & 2 & $\alpha \derive \beta $ & \centerParbox{2, \tagimpB}
		\\\hline
		7 & \tagSR & & $\dfrac{X \derive \alpha \und X,\alpha \derive \beta}{X \derive \beta}$ & & & \tagSR
		\\\hline
		8 & -1 & $X \subst \emptyset$ & $\dfrac{\derive \alpha \und \alpha \derive \beta}{\derive \beta}$ & 2, 6 & $\derive \beta $ & \centerParbox{2, 6, \tagSR}
		\\\hline
		9 & \tagnota & & $\dfrac{X \derive \alpha, \lnot\alpha} {X \derive \beta}$ & & & \tagnota
		\\\hline
		10 & -1 & $X \subst \emptyset$ & $\dfrac{\derive \beta, \lnot\beta} {\derive \alpha}$ & & & \tagnota
		\\\hline
		11 & -1 & $\alpha \change \beta$ & $\dfrac{\derive \beta, \lnot\beta} {\derive \alpha}$ & & & \tagnota
		\\\hline
		12 & -1 & $\alpha \subst \lnot\alpha$ & $\dfrac{\derive \beta, \lnot\beta} {\derive \lnot\alpha}$ & 8, 3 & $\derive \lnot\alpha$ & \centerParbox{2, 3, \tagnota, \tagimpB}
		\\\hline
		13 & \tagimpE & & $\dfrac{X, \alpha \derive \beta}{X \derive \alpha\limp\beta}$ & & & \tagimpE
		\\\hline
		14 & -1 & $X \subst \emptyset$ & $\dfrac{\alpha \derive \beta}{\derive \alpha\limp\beta}$ & & & \tagimpE
		\\\hline
		15 & -1 & $\alpha \change \beta$ & $\dfrac{\beta \derive \alpha} {\derive \beta\limp\alpha}$ & & & \tagimpE
		\\\hline
		16 & -1 & $\alpha \subst \lnot\alpha$ & $\dfrac{\beta \derive \lnot\alpha} {\derive \beta\limp\lnot\alpha}$ & & & \tagimpE
		\\\hline
		17 & -1 & $\beta \subst \lnot\beta$ & $\dfrac{\lnot\beta \derive \lnot\alpha}{\derive \lnot\beta\limp\lnot\alpha}$ & ??? & $\derive \lnot\beta\limp\lnot\alpha$ & \centerParbox{???, \tagimpE}
		\\\hline
		18 & \tagimpE+1 & $\alpha \subst \gamma$ & $\dfrac{\gamma \derive \beta} {\derive \gamma\limp\beta}$ & & & \tagimpE
		\\\hline
		19 & -1 & $\beta \subst \delta$ & $\dfrac{\gamma \derive \delta} {\derive \gamma\limp\delta}$ & & & \tagimpE
		\\\hline
		20 & -1 & $\gamma \subst \alpha\limp\beta$ & $\dfrac{\alpha\limp\beta \derive \delta} {\derive (\alpha\limp\beta)\limp\delta}$ & & & \tagimpE
		\\\hline
		21 & -1 & $\scriptstyle \delta \subst \lnot\beta\limp\lnot\alpha$ & $\frac{\alpha\limp\beta \derive \lnot\beta\limp\lnot\alpha}
		{\derive (\alpha\limp\beta)\limp(\lnot\beta\limp\lnot\alpha)}$ & 2, 17 &
		$\derive (\alpha\limp\beta)\limp(\lnot\beta\limp\lnot\alpha)$ & \centerParbox{2, ???, \tagimpB, \tagimpE}
		\\\hline\hline
		22 & & & $\dfrac{}{\derive A_1}$ & & $\derive (\alpha\limp\beta)\limp(\lnot\beta\limp\lnot\alpha)$ & \centerParbox{\tagimpB, \tagimpE}
		\\\hline
	\end{tabular}

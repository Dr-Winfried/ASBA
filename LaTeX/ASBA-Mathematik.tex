...
\begin{description}
	...
	Man sagt: $f$ ist eine $n$-stellige \Funktion\ von $A_1 \times \dots \times A_n$ \defn{in} (oder auch \defn{nach}) B (Schreibweise: $f : A_1 \times \dots \times A_n \rightarrow B$) oder, im Fall $n=1$, $f$ ist eine Funktion von $A$ in (oder nach) $B$ (Schreibweise: $f : A \rightarrow B$). Mit $A \symdefeq A_1 \times \dots \times A_n$ kann für $n > 0$ jede Funktion als $1$-stellig aufgefasst werden.
...
\begin{itemize}
	...
	%
	\item $\Gamma \bindefinition{\symderive} \Delta$ nennen wir auch eine \definition{\Ableitung} \defn{in} $\formulaSet$.
...
%%%	&  A = (\links{A},\rechts{A})
%%%	&& \text{und es gilt}
%%%	&& \links{A}, \rechts{A} \symsubseteq \formulaSet \\
...
\end{itemize}
Die \glsIdy{Schlussregel}{Schlussregel, allgemeingültig} heißt \defn{\allgemeingueltig}, wenn aus den \Voraussetzungen\ alle \Folgerungen\ abgleitet\ werden können.
...
Nur die auf diese Weise konstruierten \Formeln\ sind Elemente von $\symalForx$.
...
Ausgehend von den \Junktoren\ aus der \BoolschenSignatur\ $\symalJun_\iBool$ werden die restlichen \Junktoren\ aus $\symalJun$ definiert.

...
In diesem \sectionname\ geht es um die \glsIdxPl{zulaessigeTransformationA}, \textdh die \glsIdxBg{allgemeingueltigeSchlussregel}{allgemeingültigen Schlussregeln}. Dazu gehören zunächst die \glsIdxPl{Basisregel}.
Dann aber auch alle aus den \glsIdxPl{Basisregel} und den bis dahin \glsIdxBg{allgemeingueltigeSchlussregel}{allgemeingültigen Schlussregeln} korrekt abgeleiteten neuen Schlussregeln.
...
\begin{align}
	...
	\label{def:ableitbar}
	\\
	& \derive \beta \quad \metadefeq \quad \emptyset \derive \beta \qquad \text{(\textdh $\beta$ ist allgemeingültig.)}
	\\
...
\begin{align}
	...
	\left\{
		\begin{array}{l}
			...
			\text{Verweis auf eine \glsIdx{allgemeingueltigeSchlussregel}: Die Regel, auf die verwiesen wird.}
		\end{array}
	\right.
	\\
	S_n & \defeq \text{Die anzuwendende Substitution.}
	\\
...
\begin{table}[!htb]
	...
	\begin{tabular}{|c||c|c|c|c|c|c|}
		\hline
		\parbox{0.95cm}{\centering\textbf{Zeile}                       ($n$)} &
		\parbox{1.05cm}{\centering\textbf{Regel}                     ($R_n$)} &
		\parbox{1.85cm}{\centering\textbf{Substitu"=tionen}          ($S_n$)} &
		\parbox{1.80cm}{\centering\textbf{erzeugte Regel} ($\overline{R}_n$)} &
		\parbox{2.15cm}{\centering\textbf{angewendet auf ...}        ($Z_n$)} &
		\parbox{1.65cm}{\centering\textbf{Aussage}                   ($A_n$)} &
		\parbox{1.95cm}{\centering\textbf{Abhängig"=keiten}          ($D_n$)}
		\\\hline\hline
		1 & \parbox{1.35cm}{\centering Voraus"=setzung} & & & & $X \derive \alpha$ & $A_1$
		\\\hline
		2 & \parbox{1.35cm}{\centering Voraus"=setzung} & & & & $X,\alpha \derive \beta$ & $A_2$
		\\\hline
		3 & \parbox{1.00cm}{\centering Folge"=rung} & & & & $X \derive \beta$ & $A_3$
		\\\hline
		4 & $\MR$ & & $\dfrac{X \derive \alpha}{X, Y \derive \alpha}$ & & & $\MR$
		\\\hline
		5 & $\overline{R}_4$ & $Y \subst \lnot\alpha$ & $\dfrac{X \derive \alpha}{X, \lnot\alpha \derive \alpha}$ & $A_1$ & $X, \lnot\alpha \derive \alpha$ & $A_1$, $\MR$
		\\\hline
		6 & $\AR$ & & $ \dfrac{}{\alpha \derive \alpha} $ & & & $\AR$
		\\\hline
		7 & $\overline{R}_6$ & $\alpha \subst \lnot\alpha$ & $\dfrac{}{\lnot\alpha \derive \lnot\alpha}$ & & $\lnot\alpha \derive \lnot\alpha$ & $\AR$
		\\\hline
		8 & $\overline{R}_4$ & $\alpha \subst \lnot\alpha$ & $\dfrac{X \derive \lnot\alpha}{X, Y \derive \lnot\alpha}$ & & &  $\MR$
		\\\hline
		9 & $\overline{R}_8$ & $X \subst \lnot\alpha$ & $\dfrac{\lnot\alpha \derive \lnot\alpha}{Y, \lnot\alpha \derive \lnot\alpha}$ & & & $\MR$
		\\\hline
		10 & $\overline{R}_9$ & $Y \subst X$ & $\dfrac{\lnot\alpha \derive \lnot\alpha}{X,\lnot\alpha \derive \lnot\alpha}$ & $A_7$ & $X,\lnot\alpha \derive \lnot\alpha$ & $\AR$, $\MR$
		\\\hline
		11 & $\nota$ & & $\dfrac{X \derive \alpha, \lnot\alpha}{X \derive \beta}$ & & & $\nota$
		\\\hline
		12 & $\overline{R}_{11}$ & $X \subst X, \lnot\alpha$ & $\dfrac{X,\lnot\alpha \derive \alpha, \lnot\alpha}{X,\lnot\alpha \derive \beta}$ & $A_5$, $A_{10}$ & $X,\lnot\alpha \derive \beta$ &
		\parbox{3.0cm}{\centering $A_1$, $\AR$, $\MR$, $\nota$}
		\\\hline
		13 & $\notb$ & & $\dfrac{X,\alpha \derive \beta \und X,\lnot\alpha \derive \beta}{X \derive \beta}$ & $A_2$, $A_{12}$ & $X \derive \beta$ &
		\parbox{3.0cm}{\centering $A_1$, $A_2$, $\AR$, $\MR$, $\nota$, $\notb$}
		\\\hline\hline
		14 & & & $\dfrac{A_1 \und A_2}{A_3}$ & & $\dfrac{X \derive \alpha \und X, \alpha \derive \beta}{X \derive \beta}$ &
		\parbox{2.5cm}{\centering $\AR$, $\MR$, $\nota$, $\notb$}
		\\\hline
	\end{tabular}
...
\begin{enumerate}
	...
	\footnote{%
		...
		Sollte die bewiesene Behauptung in einer letzten Zeile angegeben werden, wird auf die Folgerungen verwiesen, so dass ihre Angabe nötig ist.
	}
	aufgeführt, für jede Voraussetzung \textbzw Folgerung eine Zeile.
	\begin{enumerate}
		\item $(n)   =$ Die aktuelle Zeilennummer $i$.
		\item $(R_n) =$ \strqt{Voraussetzung} \textbzw \strqt{Folgerung}.
	...
	\begin{enumerate}
		...
		\left\{
			\begin{array}{l}
				\text{Annahme: Die Aussage $A_i$ vorübergehend als zutreffend angenommen.}\\
		...
		Man beachte, dass die Regel $(\overline{R}_i)$, falls angegeben, stets allgemeingültig ist, da sie aus allgemeingültigen Regeln ausschließlich mittels Substitutionen abgeleitet ist.
		Daher gibt es keine Beschränkung von weiteren Substitutionen durch irgendwelche Abhängigkeiten.
		%
		...
		%
		%TODO Genau definieren, wie sich die Abhängigkeiten zusammensetzen.
		...
		%
	\end{enumerate}
	Der Beweis muss so lange fortgeführt werden, bis alle Folgerungen als Aussagen $A_i$ (für ausgewählte $i$) unter $(A_n)$ abgeleitet worden sind.
	%
	\item In der letzten Zeile kann noch die bewiesene Behauptung in der Form eines \glsIdxBg{formalerSatzV}{formalen Satzes}, \seename~\eqref{def:formalerSatz} \sectionname~\vref{sec:Schlussregeln}, formuliert werden.
	\begin{enumerate}
		%
		\item $(n) =$ Die aktuelle Zeilennummer $i$.
		%
		\item $(\overline{R}_n) =$ Die bewiesene Behauptung (mathematischer Satz) in der Form eines \emph{\glsIdxBg{formalerSatzV}{formalen Satzes}}\footnote{\seename~\eqref{def:formalerSatz} \sectionname~\vref{sec:Schlussregeln}}, wobei alle $A_i$, die Voraussetzungen sind, in dem formalen Satz als Voraussetzung und alle $A_i$, die Folgerungen sind, als Folgerung eingesetzt werden.
		%
...
%%%Zur leichteren Lesbarkeit sind einige redundante Klammern eingefügt.

\todo{Beispiel mit Reduzierung von Abhängigkeiten}%%%
%TODO Beispiel mit Reduzierung von Abhängigkeiten %%%
\begin{table}[!htb]
	\setlength\tabcolsep{1pt}
	\setlength\extrarowheight{7pt}
	\begin{tabular}{|c||c|c|c|c|c|c|}
		...
		\parbox{1.95cm}{\centering\textbf{Abhängig"=keiten}          ($D_n$)}
		\\\hline \hline
		1 & \parbox{1.00cm}{\centering Folge"=rung} & & & & $\derive (\alpha\limp\beta)\limp(\lnot\beta\limp\lnot\alpha)$ & $A_1$
		\\\hline
		2 & \parbox{1.20cm}{\centering An"=nahme} & & & & $\derive \alpha\limp\beta$ & $A_2$
		\\\hline
		3 & \parbox{1.20cm}{\centering An"=nahme} & & & & $\derive  \lnot\beta$        & $A_3$
		\\\hline
		4 & \parbox{1.20cm}{\centering An"=nahme} & & & & $\derive  \alpha$            & $A_4$
		\\\hline
		5 & $\impB$ & & $\dfrac{X \derive \alpha\limp\beta}{X,\alpha \derive \beta}$ & & & $\impB$
		\\\hline
		6 & $\overline{R}_{-1}$ & $X \subst \emptyset$ & $\dfrac{\derive \alpha\limp\beta}
		{\alpha \derive \beta}$ & $A_2$ &
		$\alpha \derive \beta $ & $A_2$, $\impB$
		\\\hline
		7 & $\TR$ ? & & $\dfrac{X \derive \alpha\limp\beta \und X \derive \alpha}
		{X \derive \beta}$ & & & $\TR$
		\\\hline
		8 & $\overline{R}_{-1}$ & $X \subst \emptyset$ & $\dfrac{\derive \alpha\limp\beta \und \derive \alpha}
		{\derive \beta}$ & $A_2$, $A_6$ &
		$\derive \beta $ & \parbox{2cm}{\centering $A_2$, $A_6$, $\TR$}
		\\\hline
		9 & $\nota$ & & $\dfrac{X \derive \alpha, \lnot\alpha} {X \derive \beta}$ & & & $\nota$
		\\\hline
		10 & $\overline{R}_{-1}$ & $X \subst \emptyset$ & $\dfrac{\derive \beta, \lnot\beta} {\derive \alpha}$ & & & $\nota$
		\\\hline
		11 & $\overline{R}_{-1}$ & $\alpha \change \beta$ & $\dfrac{\derive \beta, \lnot\beta} {\derive \alpha}$ & & & $\nota$
		\\\hline
		12 & $\overline{R}_{-1}$ & $\alpha \subst \lnot\alpha$ & $\dfrac{\derive \beta, \lnot\beta} {\derive \lnot\alpha}$ & $A_8$, $A_3$ & $\derive \lnot\alpha$ &
		\parbox{2cm}{\centering $A_2$, $A_3$, $\nota$, $\impB$}
		\\\hline
		13 & $\impE$ & & $\dfrac{X, \alpha \derive \beta}{X \derive \alpha\limp\beta}$ & & & $\impE$
		\\\hline
		14 & $\overline{R}_{-1}$ & $X \subst \emptyset$ & $\dfrac{\alpha \derive \beta}{\derive \alpha\limp\beta}$ & & & $\impE$
		\\\hline
		15 & $\overline{R}_{-1}$ & $\alpha \change \beta$ & $\dfrac{\beta \derive \alpha} {\derive \beta\limp\alpha}$ & & & $\impE$
		\\\hline
		16 & $\overline{R}_{-1}$ & $\alpha \subst \lnot\alpha$ & $\dfrac{\beta \derive \lnot\alpha} {\derive \beta\limp\lnot\alpha}$ & & & $\impE$
		\\\hline
		17 & $\overline{R}_{-1}$ & $\beta \subst \lnot\beta$ & $\dfrac{\lnot\beta \derive \lnot\alpha} {\derive \lnot\beta\limp\lnot\alpha}$ & ??? & $\derive \lnot\beta\limp\lnot\alpha$ & ???, $\impE$
		\\\hline
		18 & $\overline{R}_{\impE+1}$ & $\alpha \subst \gamma$ & $\dfrac{\gamma \derive \beta} {\derive \gamma\limp\beta}$ & & & $\impE$
		\\\hline
		19 & $\overline{R}_{-1}$ & $\beta \subst \delta$ & $\dfrac{\gamma \derive \delta} {\derive \gamma\limp\delta}$ & & & $\impE$
		\\\hline
		20 & $\overline{R}_{-1}$ & $\gamma \subst \alpha\limp\beta$ & $\dfrac{\alpha\limp\beta \derive \delta} {\derive (\alpha\limp\beta)\limp\delta}$ & & & $\impE$
		\\\hline
		21 & $\overline{R}_{-1}$ & $\scriptstyle \delta \subst \lnot\beta\limp\lnot\alpha$
		& $\frac{\alpha\limp\beta \derive \lnot\beta\limp\lnot\alpha}
		{\derive (\alpha\limp\beta)\limp(\lnot\beta\limp\lnot\alpha)}$ & $A_2$, $A_{17}$&
		$\derive (\alpha\limp\beta)\limp(\lnot\beta\limp\lnot\alpha)$ & \parbox{2.2cm}{\centering $A_2$, $???$, $\impB$, $\impE$}
		\\\hline\hline
		22 & & & $\dfrac{}{\derive A_1}$ &
		& $\derive (\alpha\limp\beta)\limp(\lnot\beta\limp\lnot\alpha)$ &
		\\\hline
	\end{tabular}
	\caption{Ableitung der \glsIdx{Kontraposition} aus \glsIdxBg{allgemeingueltigeSchlussregel}{allgemeingültigen Schlussregeln}}

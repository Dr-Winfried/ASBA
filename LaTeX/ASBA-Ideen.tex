...
\begin{align}
	...
	\label{def:ableitbar}
	\\
	& \glsSym{derive} \beta \quad \metadefeq \quad \emptyset \derive \beta \qquad \text{(\chrqt{$\derivesym$} kann dann auch ganz entfallen)}
	\\
	&                    X_1,X_2,...,X_n,\alpha_1,\alpha_2,...,\alpha_m\quad
	\glsSym{derive}\quad Y_1,Y_2,...,Y_n, \beta_1, \beta_2,..., \beta_m\quad
	\metadefeq \quad X \derive Y
	\label{def:ableitbarKurz}
	\formulatoleft
\end{align}

Eine \defn{\zulaessigeTransformation} ist die Anwendung einer \emph{\Substitution}{\vrefnotesub{sub:Identitätsregeln} (siehe unten), einer \emph{\Basisregel} (siehe unten) oder einer davon abgeleiteten sonstigen \emph{\Schlussregel}, \textzB\ aus \vrefsub{sub:Schlussregeln}.
Bei den \Schlussregeln\ und der \Substitution\ \chrGlsSym{subst} soll das Komma stärker binden als \chrGlsSym{derive}, \chrGlsSym{subst} und \chrGlsSym{srand}, wobei \chrGlsSym{srand} für \enquote{und} \textbzw\ \chrGlsSym{metaand}\vrefnotesub{sub:AussagenUndMetaoperationen} steht und schwächer bindet als \chrGlsSym{derive} und \chrGlsSym{subst}.%
\footnote{siehe Fußnote~3 \vrefvontab{tab:Prioritäten}}
...
\footnote{%
	In der \Monotonieregel\ wird hier, anders als in~\cite{bib:Rautenberg}, \seqqt{$X,Y$} statt \seqqt{$ Y \text{ , für } Y \supseteq X $} genommen. Das ist gleichwertig, vermeidet aber den Zusatz \seqqt{$ \text{ , für } Y \supseteq X $}.
...
Die \defn{\zulaessigenTransformationen}, \textdh\ die Anwendung der \Schlussregeln, erfordern zulässige \Substitutionen.
...
\begin{align}
	\gamma(\alpha \glsSym{subst} \beta) \quad \defeq \quad
	\parbox[t]{11cm}{%
	...
	\gamma(\alpha \glsSym{swap} \beta) \quad \defeq \quad
	\parbox[t]{11cm}{%
...
\seqqt{$ \alpha \glsSym{subst} \beta $} heißt \defn{\Substitution} und \seqqt{$ \alpha \glsSym{swap} \beta $} \defn{\Vertauschung} oder kurz \defn{Tausch}.
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\setcounter{Enumii}{\value{enumii}}% Nummerierung wird fortgesetzt.
	\end{enumerate}
	Man beachte, dass die \Schlussregel\ $\overline{R}_i$, stets allgemeingültig ist, da sie ausschließlich aus \allgemeingueltigenSchlussregeln\ mittels \Substitutionen\ abgeleitet worden ist.

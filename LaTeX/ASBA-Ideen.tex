...
Gemäß \cite{bib:Rautenberg} Kapitel~1.4 \emph{Ein vollständiger aussagenlogischer Kalkül} werden sechs \Basisregeln\ definiert.
Zuvor werden aber noch einige Definitionen gebraucht.
Dazu seien $n$, $m$, $k$ und $l$ natürliche Zahlen (auch~0), $\alpha$, $\alpha_i$, $\beta$ und $\beta_j$ \Formeln\, $X$, $X_i$, $Y$ und $Y_j$ Mengen von \Formeln\ und
\begin{align}
	%
...
$X$ und $Y$ können auch die \leereMenge\ sein.
...
\footnote{%
	In der \Monotonieregel\ wird \hier, anders als in~\cite{bib:Rautenberg}, \seqqt{$X,Y$} statt \seqqt{$ Y \text{ , für } Y \MtsSupsetEq X $} genommen.
	Das ist gleichwertig, vermeidet aber den Zusatz \seqqt{$ \text{ , für } Y \MtsSupsetEq X $}.
...
\end{itemize}
%
Bei obiger Definition der \Ersetzung\ bleibt noch offen, unter welchen \Praemissen\ sie angewendet werden darf.
Das soll hier nicht untersucht werden.
In diesem \sectionname\ genügt es, das nur \Vertauschung\ und vollständige \Ersetzung\ verwendet werden.
...
Zur kürzeren Darstellung wird statt auf die vollständigen Spaltenüberschriften nur auf die dort notierten $(X_n)$ verwiesen.
Dass in der Spalte $(n)$ stets die Zeilennummer steht, wird im folgenden nicht mehr extra erwähnt.
...
%%%			\multicolumn{11}{l}{~} \\
		...
		\begin{tablenotes}
			...
			%
			\item[5] Klammerung ist genau genommen keine \Operation\ und wird nicht nur bei logischen, sondern auch bei anderen Ausdrücken verwendet.
			Ihre Priorität - sofern man überhaupt davon sprechen kann - kann nur höher als die aller \Junktoren\ sein.
			%
...
\end{cases}
\end{equation}
und weiterhin gibt es eine bijektive Abbildung von \OjkFor\ nach \OjkForp.
Auf einen \Beweis\ verzichten wir.
%
...
Die \Formeln, die nach einer der Regeln \eqref{def:ForU}, \eqref{def:ForB}, \eqref{def:ForUp} oder \eqref{def:ForBp} gebildet wurden, sind offensichtlich \zerlegbar, die anderen, \textdh\ Variablen und Konstanten (aus \OjkVar\ \textbzw\ \OjkCon), sind nicht \zerlegbar.
Letztere bezeichnet man auch als \defTxt{\atomare} \Formeln.

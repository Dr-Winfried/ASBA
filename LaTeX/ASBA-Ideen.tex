...
Eine \definition{\zulaessigeTransformation} ist die Anwendung einer \emph{\Substitution}{\vrefnotesub{sub:Identitätsregeln} (siehe unten), einer \emph{\Basisregel} (siehe unten) oder einer davon abgeleiteten sonstigen \emph{\Schlussregel}, \textzB\ aus \vrefsub{sub:Schlussregeln}.
...
\begin{itemize}
	...
	\footnote{%
		...
		siehe Definition~\eqref{def:SubstitutionenAlt} \vrefvonsub{sub:Identitätsregeln}
	}.
...
\begin{table}[!htb]
	...
	\begin{tabular}{|c||c|c|c|c|c|c|}
		...
		\\\hline
		5 & \impB & & $\dfrac{X \derive \alpha\limp\beta}{X,\alpha \derive \beta}$ & & &
		\\\hline
		6 & -1 & $X \subst \emptyset$ & $\dfrac{\alpha\limp\beta}{\alpha \derive \beta}$ & 2 & $\alpha \derive \beta $ & 2
		\\\hline
		7 & \SR & & $\dfrac{X \derive \alpha \srand X,\alpha \derive \beta}{X \derive \beta}$ & & &
		\\\hline
		8 & -1 & $X \subst \emptyset$ & $\dfrac{\alpha \srand \alpha \derive \beta}{\beta}$ & 4, 6 & $\beta $ & 4, 6
		\\\hline
		9' & \andE & & $\dfrac{X \derive \alpha, \beta}{X \derive \alpha \land \beta}$ & & &
		\\\hline
		...
		\\\hline
		12 & \impE & & $\dfrac{X, \alpha \derive \beta}{X \derive \alpha\limp\beta}$ & & &
		\\\hline
		...
		\\\hline\hline
		16 & \centerParbox{1.5cm}{\impE, \impB, \SR} & & $\dfrac{}{A_1}$ & & $\dfrac{}{(\alpha\limp\beta)\limp(\lnot\beta\limp\lnot\alpha)}$ &
		\\\hline
	\end{tabular}

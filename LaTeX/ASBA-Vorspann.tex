...
% Schalter (...Flag...) und davon abhängige Makros #############################
...
\showframeFlgtrue%(nur mit \testFlgtrue)	Parameter 'showframe' nutzen
%\showidxFlgtrue%  (nur mit \testFlgtrue)	Paket     'showidx'   nutzen
%\minitocFlgtrue%                        	Paket     'minitoc'   nutzen
%\indexFlgtrue%                          	Paket  '[split]index' nutzen
%\splitindexFlgtrue%                     	Paket    'splitindex' nutzen
...
\ifsplitindexFlg
	\indexFlgtrue
	\showidxFlgfalse% nicht mit 'splitidx'
\else \fi
...
\else \fi
\ifshowframeFlg
	\geometry{textwidth=170mm,textheight=256mm,twoside,showframe}
\else % Für das fertige Dokument -----------------------------------------------
...
\newcommand*{\seqqt}[1]{\ensuremath{\left<\mkern-4mu\left<\text{#1}\right>\mkern-4mu\right>}}% Zeichenfolge/Formel
\newcommand*{\charf}[1]{\textbf{\texttt{#1}}}%Schriftart für Zeichen
\newcommand*{\strqt}[1]       {``\charf{#1}''}%              Zeichenkette
% Sonstige Quotierungen:
\newcommand*{\standsfor}[1]{\enquote{#1}}% Beschreibung eines Begriffs
...
\newcommand*{\defFt} [1]{\textbf{#1}}% Definitionen
\newcommand*{\gloFt} [1]{\textsf{#1}}% Wenn ein zu beschreibender Begriff in seiner Beschreibung mehr als einmal auftaucht

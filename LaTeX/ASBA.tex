...
% \usepackage{hypcap} % ... dies Paket aber noch dahinter - funktioniert nicht.
...
% ZELLEN vertikal zentrieren
	...
	\begingroup
		...
		\label{subsub:Bausteine}
		 ...
		 \newcommand{\ItemB}[3]{\item[]\begin{tabbing}\hspace{0.7cm}\=\hspace{4.0cm}\=\kill#1\>#2\>#3\end{tabbing}}
		 \newcommand{\ItemF}[3]{\item[]\begin{tabbing}\hspace{2.0cm}\=\hspace{4.5cm}\=\kill#1\>#2\>#3\end{tabbing}}
		...
		\begin{itemize}
			\ItemB{$\asN$}{$\defeq$}{Menge der \emph{natürlichen Zahlen} einschließlich $0$.}
			...
			\end{itemize}
			\ItemB{$\asJ$}{$\defeq\asU\cup\asB\cup\asG$}{Menge der \emph{Junktoren} und \emph{Gliederungszeichen}}
			\ItemB{$\asA$}{$\defeq\asV\cup\asJ$}{\emph{Alphabet} der logischen Sprache.}
		\end{itemize}
		Die Symbole werden noch um die weiteren in der \tablename~\vref{tab:Symbole} vorhandenen, bisher nicht verwendeten Symbole ergänzt.
		\begin{itemize}
			\ItemB{}{}{}
			\begin{itemize}
				\ItemB{$\asK$}{$\defeq\{\ltrue,\lfalse,\lntrue,\lnfalse\}$}{Menge der \emph{Konstanten}.}
				\ItemB{$\asUe$}{$\defeq\asU\cup\{\ltrue,\lfalse,\lntrue,\lnfalse\}$}{\emph{Erweiterte} Menge der unären Operatoren.}			\ItemB{$\asBe$}{$\defeq\asB\cup\{\lleftimp,\lright,\lleft,\lxor,\ltrue,\lfalse,\lnand,\lnor,\lnimp,\lnequiv,\lnleftimp,\lnright,\lnleft,\lnxor,\lntrue,\lnfalse\}$}{\\\>\>\emph{Erweiterte} Menge der binären Operatoren.}
			\end{itemize}
			\ItemB{$\asJe$}{$\defeq\asK\cup\asUe\cup\asBe\cup\asG$}{\emph{Erweiterte} Menge der Konstanten, Junktoren und Gliederungszeichen}
			\ItemB{$\asAe$}{$\defeq\asV\cup\asJe$}{\emph{Erweitertes} Alphabet der logischen Sprache.}
		\end{itemize}
		Wie man leicht sieht, ist stets $\asX\subset\asXe$ für $\asX\in\{\asU,\asB,\asJ,\asA\}$

		\subsubsection{Formationsregeln}
		\label{subsub:Formeln}

		Wir definieren rekursiv noch zwei weitere Mengen:
		\begin{itemize}
		...
		\end{itemize}
		Wie man leicht sieht, ist $\asF\subset\asFe$.

		Durch Anwendung der Klammerregeln in \sectionname~\vref{sub:Klammerregeln} lassen sich in der Regel noch die meisten Klammern einsparen. Die Namen der Operationen finden sich in der \tablename~\vref{tab:Symbole}
	\endgroup

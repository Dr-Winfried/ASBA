...
\newglossaryentry{intEigenschaftA}{
	...
	description={
		Interessierende Eigenschaften sind solche, die \textbzgl \symqt{$=$} \textbzw \symqt{$\equiv$} von Interesse sind.
		\textZB ist die interessierende Eigenschaft der Operanden der Gleichung \forqt{$y=f(x)$} normalerweise der Wert (von \forqt{$y$} und \forqt{$f(x)$}), und nicht deren Darstellung (Zeichenketten)%
	}%
}
...
\begin{document}
	...
	\footnote{%
		\emph{\glsIdxPl{Fachbegriff}} sind Namen für mathematische Elemente und Konstruktionen, \textzB \glsIdxPl{Axiom}, \glsIdxPl{Satz},  \glsIdxPl{Beweis} und \glsIdxPl{Fachgebiet}.
		Symbole können als spezielle \glsIdxPl{Fachbegriff} aufgefasst werden.%
	}
	und \emph{\glsIdxPl{Fachgebiet}n}%
	\footnote{%
		Ein \emph{\glsIdx{Fachgebiet}} ist ein Teil der Mathematik mit einer zugehörigen Basis an Axiomen, \glsIdxPl{Satz}n und spezifischen \glsIdxPl{Fachbegriff}n und Darstellungen.
	...
	\begin{enumerate}
		...
		Ist ein \glsIdx{Axiom}, \glsIdx{Satz}, \glsIdx{Beweis} oder Fachbegriff schon einmal -- \textggf abweichend -- definiert, formuliert oder bewiesen worden?
	...
	\begin{enumerate}
		...
		Es können \glsIdxPl{Fachbegriff} für \glsIdxPl{Axiom}, \glsIdxPl{Satz}, \glsIdxPl{Beweis} und \glsIdxPl{Fachgebiet} -- letztere mit eigenen Axiomen, \glsIdxPl{Satz}n, \glsIdxPl{Beweis}n, \glsIdxPl{Fachbegriff}n und über- oder untergeordneten \glsIdxPl{Fachgebiet}n -- definiert werden.
	...
	Um die Eigenschaften von \sectionname~\vref{sec:Eigenschaften} zu erreichen, werden für \ASBA\ die folgenden Ziele%
	\footnote{%
		Es sind eigentlich Anforderungen.
		Da dieser Begriff auch im \chaptername~\vref{cha:Design} verwendet wird, werden die Anforderungen hier \emph{\Idx{Ziel}}\emph{e} genannt.%
	}
	gesetzt:
	\begin{enumerate}
		...
		\footnote{%
			Um den Punkt~\ref{Eigenschaft:Ausgaben} von \sectionname~\vref{sec:Eigenschaften} erfüllen zu können, werden noch fachgebietsspezifische \glsIdxPl{Ausgabeschema} benötigt, 	welche die Art der Ausgaben beschreiben.%
		}.
		...
		Insbesondere kann zu jedem \glsIdx{Beweis} angegeben werden, wie viele Beweisschritte und welche \glsIdxPl{Axiom} und \glsIdxPl{Satz}%
		\footnote{
		...
		\glsIdxPl{Fachbegriff} und die Darstellung bei der Ausgabe können mit Hilfe von -- gegebenenfalls unbenannten -- untergeordneten \glsIdxPl{Fachgebiet}n angepasst werden.
	...
	In der \figurename~\vref{fig:Umgebung} wird beschrieben, welche Interaktionen \ASBA\ mit der Umgebung hat, \textdh welche Ein- und Ausgaben existieren und woher sie kommen \textbzw wohin sie gehen.
	\begin{figure}[h]
...
%%%			\multiput(-7.9,-3.5)(+1.0,0.0){17}{\line(0,1){ 7.0}}%%%
	...
	\begin{itemize}
		...
		\begin{description}
			...
			Die Datei wird um zusätzliche Auswertungen ergänzt, \textzB ob die \glsIdxPl{Beweis} korrekt sind, welche \glsIdxPl{Axiom} und \glsIdxPl{Satz} -- auch externe aus dem Internet -- verwendet wurden, Anzahl der Beweisschritte \textusw
		\end{description}
	...
	\begin{description}
		...
		Es können wie bei Sätzen Voraussetzungen vorhanden sein, aber keine \glsIdx{Beweis}.
		...
		Letztere können \glsIdxPl{Axiom} und andere Sätze sein, auf die dann verwiesen wird.

		\item[\glsIdxPl{Beweis}]        \label{Daten:Beweis}
		Ein \glsIdx{Beweis} besteht aus einer Folge von \glsIdxBg{Beweisschritt}{Beweisschritten}, die aus gegebenen Voraussetzungen eine Behauptung ableitet.

		\item[\glsIdxPl{Fachbegriff}]   \label{Daten:Fachbegriff}
		Ein \glsIdx{Fachbegriff} ist ein Name für ein \glsIdx{Praedikat} in einem bestimmten \glsIdx{Fachgebiet}.

		\item[\glsIdxPl{Fachgebiet}]    \label{Daten:Fachgebiet}
		Ein Fachgebiet ist ein Teil der Mathematik mit einer zugehörigen Basis von \glsIdxPl{Axiom}, \glsIdxPl{Satz}, \glsIdxPl{Fachbegriff} und \glsIdxPl{Ausgabeschema}, quasi eine untergeordnete Datenbank.
		...
		\textZB können zu einem \glsIdx{Satz} alle für einen Beweis notwendigen \glsIdxPl{Axiom} angegeben werden -- als Verweise.

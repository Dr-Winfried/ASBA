...
Alle hier\footnote{Mit \defFt{hier} ist immer dieses Dokument gemeint.} aufgeführten \Axiome, \Saetze\ und \Beweise\ sollen dazu kodiert und die \Beweise\ dann von \ASBA\ verifiziert werden.
...
An Stellen, wo eine \Benennung\footnote{Für Symbole gilt kann leider nur die Farbe geändert werden.} definiert wird, wird sie \likehyperDef{in dieser} Schriftart ausgegeben.
...
Die \defTxt{\Sprachebenen} sollten aber sorgfältig unterschieden werden:
\begin{description}
	\item[\defTxt{\Metasprache}] Hier die obere \Sprachebene: \MetaspracheDescription
	Ihre \Syntax\ und \Semantik\ wird nicht behandelt.

	\item[\defTxt{\formaleMetasprache}] Hier die mittlere \Sprachebene: \formaleMetaspracheDescription
	Ihre \Syntax\ und \Semantik\ werden im Folgenden noch entwickelt.

	\item[\defTxt{\Objektsprache}] Hier die untere \Sprachebene: \ObjektspracheDescription
...
\begin{description}
	\item[\defTxt{\Wahrheitswert}] Für die \Darstellung\ der \Wahrheitswerte\ abhängig von der \Sprachebene\ und dem logischen Wert der Aussage definieren wir:
	\begin{table}[H]
		\centering
...
\begin{description}
	\item[\Teilaussage] \TeilaussageDescription\ Umgekehrt ist $B$ dann eine \defTxt{\Oberaussage} von $A$ und man sagt dann auch, dass $B$ die \Teilaussage\ $A$ enthält.
	%
	\item[\echteTeilaussage] \echteTeilaussageDescription\ Entsprechend \defTxt{\echteOberaussage}.
	%
...
\begin{description}
	%
	\item[$\defSym{\MtsDefEquiv}$] \defTxt{\Aussagedefinition} \label{def:Aussagedefinition}
	...
	%
	\item[$\defSym{\MtsDefEq}$] \defTxt{\Objektdefinition} \label{def:Objektdefinition}
...
%%%%Im Vorgriff auf \vrefsubsub{subsub:Metadefinitionen} stehen \seqqt{$A \defSym{\MtsDefEquiv} B$} und \seqqt{$A \defSym{\MtsDefEq} B$} für \standsfor{$A$ \emph{ist definitionsgemäß gleich} $B$}, \seqqt{$A \defSym{\MtsAnd} B$} für \standsfor{$A$ \emph{und} $B$} und \seqqt{$A \defSym{\MtsOr} B$} für \standsfor{$A$ \emph{oder} $B$}.
...
Gemäß~\eqref{def:relback} \Pageref{def:relback} sind \defSymBin{\MtsNi}, \defSymBin{\MtsSupset} und \defSymBin{\MtsSupsetEq} die \Umkehrrelationen\ dazu (Sprechweisen: \emph{\dots\ enthält als Element \dots} und \emph{ist [echte] \Obermenge\ von}).
Es gelten entsprechende Gleichungen wie~\eqref{def:releq} und~\eqref{def:relbsp} \Pageref{def:releq}.
Schließlich sind \defSymBin{\MtsInN}, \defSymBin{\MtsSubsetN}, \defSymBin{\MtsSubsetEqN}, \defSymBin{\MtsNiN}, \defSymBin{\MtsSupsetN} und \defSymBin{\MtsSupsetEqN} gemäß~\eqref{def:relnot} \Pageref{def:relnot} noch die zugehörigen \Negationen.
...
%TODO überarbeiten, Dopplungen meiden, mit Glossar abgleichen; eindeutige Bezeichnungen
	...
	\footnote{\label{def:Junktor}%
		...
		%TODO zusätzlichen Unterabschnitt für Kombinationen von Operationen und Relationen einfügen.
...
\vrefDtab{tab:Prioritaeten} listet zur Vermeidung von Klammern die Prioritäten der in diesem Dokument verwendeten \Operationen, \Relationen, \Junktoren\ und \Metadefinitionen\ in absteigender Folge von höherer zu niedrigerer Priorität, \textdh\ von starker zu schwacher Bindung auf.%
...
\begin{table}[p]
	...
	\begin{threeparttable}
		...
		\begin{tablenotes}
			...
			\item[6] \vrefseesubsub{subsub:Metadefinitionen}
